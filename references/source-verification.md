...
- 答案侧的指纹必须对得上：`answers.json` 的 `answer_source_sha256` 要等于台账里 `role=answers` 那份原件的 sha256。若解析表其实来自另一份文件（哪怕答案恰好相同），构建会以 `answer_key_not_registered` 阻断——"答案原件 → answers.json"这一环必须能被证明，不能只看答案数值是否巧合一致。
...
最后写qa-report.md：先跑 `python3 scripts/qa_report.py init OUTPUT` 生成待填清单（它已把答案审计计数、review/blocking 逐题项、静音自动分段段落、浏览器已执行与跳过的操作、待核项落成条目），再逐条在 `结论：` 后写明判断，最后 `python3 scripts/qa_report.py check OUTPUT` 必须返回 `ok`——空结论、`待填` 之类占位文字、缺章节都会被拒绝，`package_lesson.py` 也会因此拒绝打包。分别说明：原件识别与答案逐题比对、解析自洽复核、每段/每题首尾与题意核对、浏览器实际操作（包括回填/精听/词句/写作原题）。保留未完成项及具体位置。不把“网页打开了”写成所有交互验收。无可靠转写工具时先解决环境；不能降级为复制整段音频并宣布完成。
