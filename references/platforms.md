...
这个 Skill 的脚本在 Windows、macOS、Linux 上跑同一套 Python 代码；差异集中在「外部工具怎么装」和「手机怎么打开成品」。开工先跑 `python3 scripts/doctor.py --minutes 听力时长`，它会按当前系统给出该走哪一档。
...
2. **控制台编码**：中文 Windows 控制台默认 cp936，英文系统是 cp1252，直接打印中文会 `UnicodeEncodeError`。所有脚本在入口处强制 UTF-8 输出，中文报错信息不会再崩。
...
- 路径含空格或中文时，命令里用双引号包住，例如 `python3 scripts/build.py "D:\课件\exam.json" "D:\输出\lesson"`。

## 二、手机打开成品

### 1. 正确的打开方式

1. 手机要拿到**整个文件夹**，不只是 `index.html`：`index.html` 与 `audio/`、`assets/` 是分开的，只发一个 HTML 文件过去，听力必然播放不了。
...
iOS 14.5 / Android Chrome 88 及以上最稳妥。低于这个版本仍会尽量降级运行，但弹窗与自适应布局可能出现位置偏差。
