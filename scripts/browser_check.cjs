...
async function main(){
 ...
 browser=await pw[report.engine].launch({headless:true,...(report.engine==='chromium'&&process.env.CHROME_BIN?{executablePath:process.env.CHROME_BIN}:{})});report.browser_version=browser.version();report.platform=process.platform;
