# 1.0.117 Windows CI 抓到的测试脚手架问题（推上 GitHub 后的第一条真机信号）

## 本次实际执行

本轮把 1.0.116 推上 GitHub（`hututu-ai/english-exam-studio`；父提交取线上 main HEAD，**未强推、历史与 tag 保留**）。推完线上 CI 立刻给出两条真信号。

- **Windows 格挂了**（`windows-latest` × Python 3.9/3.11 跑 `unittest discover`）。查下来是 1.0.114–1.0.116 新增测试里的**脚手架假定 POSIX**，不是产品代码：
  1. `tests/test_gates_fire.py` 与 `tests/test_quality_gate_gates.py` 各写了一个 `#!/bin/sh` 的 ffprobe 桩——Windows 没有 `/bin/sh`，那 6 条用例必然失败；
  2. `tests/test_whisper_setup.py` 断言解压后保留执行位（`st_mode & 0o100`）——Windows 没有 POSIX 执行位概念（`chmod` 只影响只读标记）。
  修法：把那两个"假 ffprobe"从**可执行脚本**改成**给被测模块的 `subprocess.run` 打桩**（`quality_gate.subprocess.run` / `build.subprocess.run`）：不依赖 `/bin/sh`、也不依赖可执行位，Windows 上不存在执行语义的差异——这样连"换一种可执行形式"的平台风险也一并去掉；执行位断言在 `os.name=='nt'` 时如实 `skipTest` 并写明原因。**这正是"Windows 必须能用"要抓的东西——而且抓出来的是我自己测试脚手架里的毛病。**
- **浏览器作业（3 个格）挂在打包步骤**：线上工作流仍是 1.0.13 时代那一版，它跑完浏览器验收后直接 `package_lesson.py`；而本项目自 1.0.44 起要求 `qa-report.md` 人工复核清单（打包两道门都要过）。这是**工作流过期**，不是产品缺陷——新版工作流已含"填清单 + 冒烟报告"两步，需要更新 `.github/workflows/compatibility.yml`。
- **一处外部限制（如实记）**：本次推送所用凭据只有 `repo` 权限，而 GitHub 硬性要求 `workflow` 才能改动 `.github/workflows/*`；因此这一版**保留线上的 CI 文件**（其余文件全部更新，线上文件数 137），等权限补齐后再补一个提交更新 CI。
- **推送方式（无 git 可用时的一条路）**：本沙箱里 `github.com` 的 git 端点不通（实测 75 秒超时），但 `api.github.com` 与 `uploads.github.com` 可用，因此改用 Git Data API：blobs → tree（带 base_tree，保留线上已有文件）→ commit（父提交＝线上 HEAD）→ 更新 ref，并在 `uploads.github.com` 上传三个发布资产。
- **读回核对**：线上 `HEAD` 的父提交是 1.0.13 时代提交；`VERSION` 与本地逐字节一致；抽查 `SKILL.md`/`README.md`/`install-manifest.json`/`scripts/build.py`/`一键体检.bat`/`docs/VERIFICATION.md` 的 blob sha 全部一致；`english-exam-studio.zip`（2,159,465 字节）、`english-exam-demo.zip`（172,170 字节）、`SHA256SUMS.txt` 的线上指纹与本地一致；被保留的 CI 文件 blob sha 等于线上原版。
- **实测**：`release_check.py` 五项全过（安装 `complete` · **135 文件** · 1.0.117；整链重放 9/9）；全套 **598 项测试通过**（默认那次 6 项如实跳过：5 项因为本机当次没设 `PLAYWRIGHT_MODULE` 而跳过真实浏览器回归、1 项因为该夹具课件没有媒体文件；把 `PLAYWRIGHT_MODULE` 指向本机已装的 Playwright 再跑一遍，那 4 项真实浏览器回归会真的执行并通过——两次的实际输出都记在下面的"复核"一节）。

## 同一版本内补做：把浏览器作业放回 CI（非阻塞），并纠正一处过期数字

第一轮因为凭据缺 `workflow` 权限而**保留**了线上旧工作流，留下一个我不满意的后果：我本地的新版工作流把 `browser` 作业整段删掉了——那等于把"Windows 上的交互验收是红的"这件事**藏起来**，而"Windows 能用"恰恰是这个项目的第一要求。删掉比修好省事，但方向是错的，所以改回来。

- **`browser` 作业恢复，且写明是证据不是门禁**：三个格分别是 ubuntu+chromium、macos+webkit、windows+chromium（Windows 用 runner 自带的 Edge 当 Chromium 内核），跑 `tests/make_browser_fixture.py` 生成**示例**产物 → `node scripts/browser_check.cjs work/browser-output --stress` → 上传 `index.html` 与 `browser-check.json` 作为可下载证据。作业级和步骤级都 `continue-on-error: true`：步骤级是为了让"这一步失败之后"的清单与产物照样跑完——过去看不到 Windows 到底挂在哪，正是因为失败即终止、连 `browser-check.json` 都拿不到。
- **不再让 CI 假装交付**：旧作业跑完浏览器检查直接 `python scripts/package_lesson.py` 打包，而本项目自 1.0.44 起要求人工复核清单（`qa-report.md`）已逐条填写——那是**人的结论**，CI 填不出来也不该代填。现在这一步换成打印清单（`qa_report.py init`，谁都能看到还差哪几条），交付判定仍留在真机真卷。
- **文档同步改真**：`docs/WINDOWS.md` 第 9 节原来写"CI 里**没有**跑浏览器交互"（那是我删掉作业后才自洽的说法），现在改成"有一个**非阻塞**的 `browser` 作业，它证明的是交互代码在示例产物上点得通，不能证明你的试卷和真机能用"，并把"CI 能证明的／不能证明的"两份清单按四个作业重写。
- **在案未决（同时写进 `docs/WINDOWS.md`，不只写在这里）**：线上 CI 在 `browser (windows-latest, chromium)` 的 `--stress` 一步报过失败，最早的记录见 v1.0.13 的那次运行，**早于本仓库当前维护者的改动，原因尚未定位**；macOS / Linux 同一作业历史结果为绿。因此该作业保持非阻塞，但**不把 Windows 的浏览器交互记为已验证**。
- **纠正一处过期数字**：原来这里的"134 文件"是删除 `tests/ffprobe_stub.py`（改用给 `subprocess.run` 打桩）**之前**的数字，删掉后是 133；本节新增静态闸门（见下）后又是 134。数字会随每次增删变动，所以这份记录只认"发布那一刻命令的实际输出"，不把历史数字当事实——写错比不写更坏，它会让后来的人以为包里有一个并不存在的文件。这条也算 1.0.117"Windows CI 抓问题"的延续：抓出来的都是我自己写的东西。

## 同一版本内再补做：把"Windows 上会炸的写法"变成本机就能跑的闸门

这一节直接对着问题①（Windows 必须能用）的根因下手。1.0.116 那次 CI 红灯之所以只能靠**推上去**才发现，是因为那类写法在 macOS 上永远是绿的；而本项目已经吃过一次"推一次、等几分钟、才知道红"的亏。所以把它改成静态闸门。

- **`tests/test_windows_portability.py`（新增，17 项）**：用 AST 扫**随发布包发给老师的每一个 `.py`**（清单复用 `package_skill.shipped_files()`，与打包口径同一份，避免"扫的文件不是发出去的文件"）。三条规则，都对着真实故障形态：
  1. `POSIX_API`——`os.fork`/`os.killpg`/`os.setsid`/`os.geteuid`/`signal.SIGKILL` 这类 Windows 上不存在的接口，或 `import pwd/grp/fcntl/termios/tty`；
  2. `EXEC_BIT`——`chmod` 置执行位、从 `st_mode` 取执行位、`stat.S_IEXEC` 之类（1.0.116 两处红灯正是这一类）；
  3. `POSIX_SHELL`——通过 `subprocess.run`/`Popen`/`os.system` 调用 `sh`/`bash` 或直接跑 `.sh`（"Windows 上没有 /bin/sh"）。
  同一个函数里必须有 `os.name` / `sys.platform` 判断或 `skipTest` 跳过，否则本机就失败。规则只认 AST 节点、不认注释和字符串，所以夹具里造 tar 元数据的 `0o755`、把 `'.sh'` 当搜索词的写法都不会误报（两条都有专门用例钉住）。
- **规则牙齿的证明**：反向用例逐条注入（不判 guard 的 `killpg`、`chmod(0o755)`、`st_mode & 0o100`、调 `/bin/sh`）必须被抓；有 guard 或 `skipTest` 的不许报；**guard 必须与坏写法在同一函数**（防"随便哪里写一句 `os.name`"骗过闸门）；还有一条要求每条规则代号都被某条反向用例断言过（只列规则不验规则 = 没有规则），以及"排除清单里的文件必须存在"（防改名后静默空转）。另有一次**真实注入验证**：把 1.0.116 那个 `stub_ffprobe`（`write_text('#!/bin/sh…')` + `chmod(…|stat.S_IEXEC)`）原样放进 `tests/`，闸门报出 1 条 `EXEC_BIT`，删掉后回到 0 条。
- **顺手改掉一处真依赖**：`scripts/prepare_whisper.py` 解压 tar 后 `if info.mode&0o111:p.chmod(0o700)` 没有判断平台——Windows 上 `chmod` 只影响只读标记，这一步本无意义。现改为显式 `if os.name!='nt' and …`，让"执行位是 POSIX 概念"这句话写在代码里，而不是靠"chmod 在 Windows 上恰好不报错"。这样闸门不需要任何豁免（除了它自己那张规则表）。
- **顺带把打包口径提成函数**：`package_skill.shipped_files(root)` 从 `package()` 里提出来复用——闸门与打包必须是同一份文件清单，否则闸门会看着老师收不到的文件报错、或漏掉真正发出去的文件。

这一段仍属 1.0.117：它改了随包发布的文件，所以没有新开版本号，而是再重跑整套发布仪式（重新生成清单 → 打包 → `release_check` → 整套测试 → 解包再跑一遍 → 重算 `SHA256SUMS.txt`）。

这一段仍属 1.0.117：`docs/WINDOWS.md` 是随包发布的文件，改了它就必须重新打包，所以没有新开版本号，而是重跑整套发布仪式（重新生成清单 → 打包 → `release_check` → 整套测试 → 解包再跑一遍 → 重算 `SHA256SUMS.txt`）。

## 同一版本内三补：把"从未被任何测试触发过"的显式判据补齐（24+10 条）

这一节对着用户的第 7 条要求（文化背景／篇章结构／生词难词／听力挖空都不许 AI 乱编）：**判据不能只是写在代码里，必须每条都有反向验证，否则改松了没人知道**。1.0.115/1.0.116 已用 `trace` 把 `build`/`grounding`/`culture` 的 `assert` 补齐；这次轮到**显式 `raise`**。

- **先量后补**：把"每条判据的说明是否出现在测试里"的覆盖面从交付三文件（`verify_output`/`package_lesson`/`qa_report`）扩到**全部 41 个随包发布的脚本**，一次量出 **24 条从未被任何测试提到过**：`prepare_whisper` 6、`audio` 4、`import_timed_text` 3、`extract_package` 2、`preferences` 2，以及 `scope`/`plan`/`build`/`parts`/`fetch_package`/`quotes`/`image_pages` 各 1。加上 `package_lesson` 那条"ZIP 完整性检查失败"（要压缩层真的出错，显式豁免并写明理由），共 25 条。
- **逐条真实触发**（新增 `tests/test_raise_gates.py`）：时间预算到点、压缩包里不安全路径、2GB 体积上限（真实 2GB 夹具造不出来，用声明体积的假 infolist，走的仍是真分支）、旧运行时不覆盖、CLI 预算范围、外部命令失败、空 WAV、**截断 WAV**（真造一个"声明帧数大于实际数据"的文件）、**切点引文与转写不匹配**、字幕坏时间行/缺文字/无时间段、解压体积上限、备份目录与技能目录重叠、没先问老师、没记录全部功能选择、选不出板块、功能编号越界、空 WAV 进构建、分节文件 ID 与文件名不符、包版本不符、引用指向不存在的段落、目录里没有图片。
- **口径也被反向验证咬了一次**：最早照搬"一律 5 字窗口"的旧口径，做变异验证时**没抓住**——塞进脚本的 `ZQPROBE9917` 被判成"已被提到"，因为 5 字窗口 `PROBE` 命中了测试里的 `FFPROBE_BIN`。改成"中文按 5 字、ASCII 按 8 字"后，又揭出 **10 条假通过**（靠 `DOCX `、`files`、`SRT /` 这类通用 ASCII 片段撞上的，等于没覆盖）：`prepare_whisper` 缺编译工具、`check_install` 清单形状、`answers`/`extract` 的 DOCX 缺 `w:body`、`align_whisperx` 计划缺字段、`audio` 空转写／未知来源转写／ASR 超时、`import_timed_text` 纯文本、`fetch_package` 缺 files 清单。这 10 条也逐条补了真实触发用例，**严格口径下缺口归零**。
- **守卫自己的反向验证做成了用例**（不是手跑一次就算）：临时把一条罕见字写成的判据塞进 `scripts/parts.py`，`unmentioned_messages()` 必须报出来，然后逐字节还原并断言还原后无问题。变异串在**运行时拼出来**——直接写成字面量的话它会出现在测试文件里，守卫于是"自己骗自己"（上一版正是这么翻的车）。
- **顺手补上一个"写了但没人核对"的数字**：`check_docs.py` 只认 README 里的 `测试总数 **N**` 与 `**N 项测试` 两种写法，而 README 原来写的是"**测试**：N 项通过"，所以那句数字**从来没被核对过**。现在改成可识别的写法，并当场做了一次变异验证：写成 596（实际 597）时 `check_docs` 立刻报"README 最新版本小节写'测试 596 项'，实际是 597"。

（同样属于 1.0.117：改了随包发布的测试与文档，所以再次重跑整套发布仪式。）

## 同一版本内四补：让"Windows 上到底卡在哪"能被诊断出来

线上 Windows 格那个**未定位**的浏览器失败，最难受的地方不是它红，而是**红得没信息**：旧报告超时只写一句
`Browser check exceeded 180 seconds.`，而产物下载还要凭据，于是谁也说不出它卡在哪一步。这一节不猜原因，只把"下一次能查"做出来。

- **超时可配**：新增 `BROWSER_CHECK_TIMEOUT`（默认仍 180 秒）。Windows 的 CI runner 比开发机慢得多，
  把上限写死会把"这台机器慢"误报成"课件坏了"。文档里同时写明：**放宽前先看失败信息**，别把真的卡死当"只是慢"。
- **超时报错带进度**：改成"已完成 N 项、最后一项是什么、当前阶段是什么"，例如
  `浏览器验收超过 180 秒还没跑完（已完成 12 项，最后一项是 …；当前阶段 stress）`。
- **报告带"体检项"**：新增 `started_at`、`elapsed_seconds`、`phase`、`timeout_seconds`、`node_version`；
  在启动、进入检查、进入压力模式、每种恢复模式前更新 `phase`，并落盘一次——
  这样即使进程被硬杀，磁盘上也留着一份"跑到哪"的现场。`started_at` + `html_sha256` 也顺手解决"这是不是旧报告"的疑问。
- **CI 直接把报告打进日志**：`browser` 作业新增一步 `if: always()` 打印 `work/browser-output/browser-check.json`
  （没有就列目录）。以后**不用下载产物、不用凭据**，点开这一次运行就能看到失败原因与完整堆栈。
- **实测**：新增 1 项真实浏览器用例（`BROWSER_CHECK_TIMEOUT=1` 必然超时），断言报告里有进度、阶段、起始时间与 Node 版本；
  正常路径重跑仍是 `passed=30 skipped=3`（未受改动影响），本机 22.6 秒。
- **仍未解决**：Windows 上那次失败的原因**依旧未定位**——这一节做的是"下次能查"，不是"已经修好"。
  在真机或 CI 给出结论之前，Windows 的浏览器交互继续记为未验证。

## 复核（本记录写完后重打包）

- 本记录写在上面的运行之后，故重新打包 1.0.117 并重跑 `release_check.py`：五项仍全过、`check_install=complete`；`SHA256SUMS.txt` 条目与新 ZIP 指纹一致。
- 补做浏览器作业与文档后的**第二次**重打包同样按这套走：清单文件数、`release_check` 五项、整套测试数、解包内自检与 `SHA256SUMS.txt` 都以命令实际输出为准（本节不写死数字，任何一处对不上就等于这次没有复核）。
- 新增跨平台静态闸门后的**第三次**重打包：`check_docs` 一致 · 清单 **134 文件** · `release_check` 五项全过（整链重放 8 passed / 0 failed / 1 skipped）· 全套 **560 项**通过（默认 5 项跳过）。另外把 `PLAYWRIGHT_MODULE` 指向本机已装的 Playwright、`BROWSER_ENGINE=chromium`、`CHROME_BIN` 指向本机 Chrome 再跑一遍：**560 项、只剩 1 项跳过**——原先"跳过真实浏览器回归"的 4 项这次真的开了浏览器、点了页面并全部通过（多花约 53 秒）。发布包解压后再跑 `check_install`（complete）与同一套 560 项，结果一致。
- 加"超时可配 + 报告带进度 + CI 打印报告"后的**第五次**重打包：`check_docs` 一致（README 的测试总数 598 已核对）· 清单 **135 文件** · `release_check` 五项全过 · 全套 **598 项**通过（默认 6 项跳过；带 Playwright 时 598 项、只剩 1 项跳过）· 解包内 `check_install=complete` 且同一套 598 项通过 · `SHA256SUMS.txt` 校验通过。
- 补齐 24+10 条显式判据后的**第四次**重打包：`check_docs` 一致（README 的测试总数这次是真的被核对了）· 清单 **135 文件** · `release_check` 五项全过 · 全套 **597 项**通过（默认 5 项跳过；带 Playwright 时同样 597 项、只剩 1 项跳过）· 解包内 `check_install=complete` 且同一套 597 项通过 · `SHA256SUMS.txt` 校验通过。

## 未覆盖边界

- 本机是 macOS：**Windows 的修复只能由 CI 复核**（这正是 CI 的价值），修复后的 Windows 格结果待下一次运行确认。
- 工作流已在本地改为"恢复 `browser` 作业（非阻塞）＋补 `smoke` 作业"，但**线上 CI 文件仍是旧版**（凭据缺 `workflow` 权限），所以线上浏览器作业在推送新工作流前仍会红；这不是产品回归。推送后要看的是：Windows 的 `--stress` 到底挂在哪（这次能拿到产物了）。
- `raw.githubusercontent.com` 在本沙箱不通（老师那边通常可用），故 `fetch_package.py` 的该通道未在本地复核。
- 真实试卷、豆包工作/WorkBuddy 真机仍未验收（同前几版声明）。
