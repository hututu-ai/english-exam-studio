...
1. 打开 Skill 文件夹里的 `VERSION` 文件，里面写着版本号（例如 `1.0.12`）。
2. 让助手运行：
...
本版加入启动恢复、真实浏览器点击检查、独立打开说明与交付 ZIP 核验。更新 Skill 后新生成课件使用新版；已经生成的旧 HTML 不会自动改变。要修复旧课件，请保留 exam.json 与媒体，使用新版固定模板重新构建并验收，勿让 Agent 从截图重写页面。
